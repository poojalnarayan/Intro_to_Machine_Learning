...
def compute_gradient_numerical_estimate(J, theta, epsilon=0.0001):
    ...
    for i in range(len(theta)):
        theta1 = list(theta)  #  theta + epsilon
        theta2 = list(theta)  # theta - epsilon
        theta1[i] = theta[i] + epsilon
        theta2[i] = theta[i] - epsilon
        cost1, grad1 = J(theta1)
        cost2, grad2 = J(theta2)
        gradient[i] = ((cost1 - cost2) / (2 * epsilon))     #lecture 25 slide 15, observe the epsilon has to be small!
