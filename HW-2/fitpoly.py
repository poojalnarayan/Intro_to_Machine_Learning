...
def fitpoly(x, t, model_order):
    ...
    #### YOUR CODE HERE ####
    w = numpy.dot(numpy.dot(inv(numpy.dot(X.T, X)),X.T),t)
...
def fit_womens100_data(model_order=1, scale_p=False, plot_p=False):
    """
    The data/womenmens100.csv data is provided just for fun, not required for HW 2.
    This function calls the "top-level" read_data_fit_plot script using the
    ...
    read_data_fit_plot(data_path, model_order=model_order, scale_p=scale_p, plot_p=plot_p,plot_title='Women’s 100m gold medal Olympic- First orderBest fit model')

def fit_synth_data(model_order=1, scale_p=False, plot_p=False):
    """
    The data/synthdata2017.csv data is provided just for fun, not required for HW 2.
    This function calls the "top-level" read_data_fit_plot script using the
    path synthetic data.
    ...
    :return:
    """
    data_path = os.path.join(DATA_ROOT, 'synthdata2017.csv')
    read_data_fit_plot(data_path, model_order=model_order, scale_p=scale_p, plot_p=plot_p,plot_title='Synthetic data -Third order polynomial function model')


# Example executions of the script -- uncomment to run:
#fit_mens100_data(model_order=1, scale_p=False, plot_p=True)
#fit_womens100_data(model_order=1, scale_p=False, plot_p=True)
#fit_synth_data(model_order=3, scale_p=False, plot_p=True)
