...
def run_cv( K, maxorder, x, t, testx, testt, randomize_data=False, title='CV' ):
    ...
    for p in range(maxorder + 1):
        ...
        for fold in range(K):
            ...
            ### YOUR CODE HERE
            w = numpy.dot(numpy.dot(inv(numpy.dot(trainX.T,trainX)),trainX.T),traint)
            ...
            #print("train_pred.shape ", train_pred.shape) pooja
...
def run_cv_modified( K, maxorder, x, t, randomize_data=False, title='CV' ):
    """
...
def run_demo_modified():
    """
    Top-level script to run the cv demo
    """
    data = numpy.genfromtxt('synthdata2017.csv', delimiter=',', dtype=None)
    x = data[:, 0]
    t = data[:, 1]
    ...
    run_cv_modified( K, 7, x, t, randomize_data=True, title='{0}-fold CV'.format(K) )
# -------------------------------------------------------------------------
# SCRIPT
# -------------------------------------------------------------------------
